...
def heatmap_by_cluster(data_categories,
                       ...
                       cmap='coolwarm', spacing=True, size=(25, 10)):

    data_categories = data_categories.sort_values(cluster_lab)
    row_color_series, categories, mapper = _gen_row_colors(
        data_categories, n_colors, cluster_lab)
    ...
    if len(data_ts > 3800):
        axvlines(3600, ax=cm.ax_heatmap, linewidth=5,
                 color='k', label='Citalopram Administration')
    if len(data_ts > 7400):
        axvlines(7200, ax=cm.ax_heatmap, linewidth=5,
                 color='r', label='WAY 100165 Administration')
